...
- **자동 업데이트** — 새 버전이 나오면 상단에 알림 배너가 뜹니다. 사용자가 **업데이트** 를 눌러 승인하면 진행률을 보여 주며 내려받고, 설치가 끝나면 원하는 때에 **지금 다시 시작** 으로 적용합니다. 툴바의 업데이트 버튼(또는 macOS 앱 메뉴 › 업데이트 확인…)으로 직접 확인할 수도 있습니다.
...
>
> 툴바 오른쪽의 세 버튼으로 탐색기 · 에디터 · 미리보기 패널을 켜고 끕니다 (`Cmd/Ctrl+B` · `Cmd/Ctrl+Shift+E` · `Cmd/Ctrl+Shift+V`). 마지막 하나 남은 패널은 숨겨지지 않으며, 선택은 다음 실행에도 유지됩니다.
